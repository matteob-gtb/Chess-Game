...
void main() {
	...
	const float dst = 0.5;
  	...
  	float bright = floor(mod(fragPos.x * dst, 1.0)+.5) + floor(mod(fragPos.y * dst, 1.0)+.5) + floor(mod(fragPos.z * dst, 1.0)+.5);

  	vec3 color = mod(bright,2.0) > .5 ? vec3(0.866,0.886,0.929) : vec3(0.15,0.12,0.1); 
