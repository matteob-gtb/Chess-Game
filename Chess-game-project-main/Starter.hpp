...
public:
	enum  DIRECTION { NORTH = 1, SOUTH = 2, EAST = 3, WEST = 4, NORTHEAST = 5, NORTHWEST = 6, SOUTHEAST = 7, SOUTHWEST = 8, Z_UP = 9,Z_DOWN = 10,  INVALID = 9 };
...
protected:
	...
	void mainLoop() {
		...
		while (!glfwWindowShouldClose(window)) {
			...
			//std::cout << "FPS: " << 1.0 / std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startTime).count() << std::endl;
	...
	void getSixAxis(float& deltaT, glm::vec3& m, glm::vec3& r, bool& fire, DIRECTION& chosenDir, bool& enterPressed) {
		static auto startTime = std::chrono::high_resolution_clock::now();
		...
		m.x = ScrollWheel::y_off;
		ScrollWheel::y_off = 0.0f;
		...
		//WASD to select a different piece 
